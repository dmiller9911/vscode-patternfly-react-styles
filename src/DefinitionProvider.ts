...
import { getCurrentLine, findImportPath, genImportRegExp } from './util';
import * as path from 'path';
import * as fs from 'fs';
import { formatClassName } from '@patternfly/react-styles/dist/js/utils';
...
function getPosition(filePath: string, className: string): Position | null {
  ...
  const classTransformer = getTransformer();
...
export class CSSDefinitionProvider implements DefinitionProvider {
  public provideDefinition(
    ...
    const matches: any = genImportRegExp('(\\S+)').exec(currentLine);
...
export default CSSDefinitionProvider;
